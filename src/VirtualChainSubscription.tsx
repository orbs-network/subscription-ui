...
class VirtualChainSubscription extends React.Component<
  VirtualChainSubscriptionProps,
  VirtualChainSubscriptionState
> {
  ...
  hasPendingTransactions(): boolean {
    return (
      (this.state.approveTxHash || this.state.subscribeTxHash) !== undefined
    );
  }
  ...
  render() {
    ...
    if (this.state.validationError) {
      return <div>Error: {this.state.validationError}</div>;
    }
      ...
      return (
        <form noValidate autoComplete="off">
          ...
          <Select
            ...
            name="subscription"
          >
            <option value={config.minimalSubscriptionAmount}>
              One month - {config.minimalSubscriptionAmount} ORBS
            </option>
            <option value={config.minimalSubscriptionAmount * 2}>
              Two months - {config.minimalSubscriptionAmount * 2} ORBS
            </option>
            <option value={config.minimalSubscriptionAmount * 3}>
              Three months - {config.minimalSubscriptionAmount * 3} ORBS
            </option>
          </Select>
  ...
  async subscribe() {
    ...
    const erc20 = new web3.eth.Contract(
      ERC20ABI as any,
      // TODO : ORL : Move this method to the store&service
      // config.erc20Address,
      ""
    );
    ...
    const amountAsNumber =
      this.state.subscriptionAmount * Math.pow(10, config.decimals);
    ...
    const subscribeTx = subscription.methods
      .subscribeForCurrentMonth(
    ...
    const distributeTx = subscription.methods
      .distributeFees()
